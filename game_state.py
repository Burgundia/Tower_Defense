...
class GameState(StateBase):
    ...
    def kill_game(self):
        ...
        if self.gamer.life <= 0:
            ...
            self.application.ending_message = 'Congratulations! Your scores: '\
                                              + str(self.gamer.money)
    ...
    def activate_many_creeps(self):
        if len(self.world.creeps) == 1 and self.world.creeps[0].current_pos \
                == self.world.ways[0][1]:
    ...
    def update_level(self, world):
        if self.wave_is_over(world) and self.world.made_creeps > 7 + \
                        self.world.wave * 2:
            world.update()
    ...
    def pack_creeps(self):
        ...
        for creep in self.world.creeps:
            ...
            texts.append((str(creep.health), creep.rect[0]+5, creep.rect[1]-5,
                          'freesansbold.ttf', 12))
            if isinstance(creep, CreepHelper):
                x = creep.rect[0] + 16
                y = creep.rect[1] + 16
            ...
            if creep.doctored:
                texts.append(('+10', creep.rect[0] + 5, creep.rect[1] - 12,
                              'freesansbold.ttf', 12, (0, 255, 0)))
    ...
    def pack_towers_and_features(self):
        ...
        for tower in self.world.towers:
            ...
            circles.append((tower.radius.x, tower.radius.y, tower.radius.size,
                            tower.radius.color))
            if tower.attacked_creep is not None:
    ...
    def pack_texts(self):
        ...
        texts.append((self.gamer.get_info()[0], 820, 100, 'freesansbold.ttf',
                      18, (0, 155, 0)))
        texts.append((self.gamer.get_info()[1], 820, 120, 'freesansbold.ttf',
                      15, (100, 100, 250)))
        texts.append(('tower type:', 815, 350, 'freesansbold.ttf', 15,
                      (0, 1, 99)))
        if self.world.current_tower_type == 'laser':
            color = (255, 0, 0)
        else:
            color = (0, 0, 255)
        texts.append((self.world.current_tower_type, 820, 370,
                      'freesansbold.ttf', 15, color))
    ...
    def pack_data(self):
        ...
        img, texts, circles2 = self.pack_creeps()
        images += img
        circles += circles2
    ...
    def handle_mouse_events(self, mouse):
        for tower in self.world.towers:
            if tower.pressed(mouse) \
                    and self.world.current_tower_type == tower.type \
                    and not self.world.put_mine:
                if self.is_enough_money(self.world.upgrade_cost, self.gamer) \
                        and tower.can_be_upgraded():
